"""Langflow custom component: Route Single Post Processing."""
...
class RouteSinglePostProcessingComponent(Component):
    display_name = "Route Single Post Processing"
    description = "Expose direct-response vs post-analysis branch for a single retrieval result."
    documentation = "https://docs.langflow.org/components-custom-components"
    icon = "GitFork"
    name = "route_single_post_processing"

    inputs = [DataInput(name="state", display_name="State", info="Single retrieval state after execute_jobs")]
    outputs = [
        Output(name="direct_response_state", display_name="Direct Response State", method="direct_response_state", group_outputs=True, types=["Data"], selected="Data"),
    ...
    def _resolve(self) -> Tuple[Dict[str, Any], str]:
        ...
        from manufacturing_agent.services.runtime_service import route_single_post_processing
        ...
        self.status = f"Single post-processing route: {route or 'inactive'}"
        self._cached = (state, route)
        return self._cached

    def direct_response_state(self):
        _ensure_repo_root()
        from langflow_version.component_base import make_branch_data

        state, route = self._resolve()
        return make_branch_data(route == "direct_response", {"state": state})
