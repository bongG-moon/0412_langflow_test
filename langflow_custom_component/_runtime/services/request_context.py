...
def collect_current_source_dataset_keys(current_data: Dict[str, Any] | None) -> List[str]:
    ...
    if isinstance(current_datasets, dict):
        dataset_keys = [raw_dataset_key(key) for key in current_datasets.keys() if str(key).strip()]
        if dataset_keys:
            return list(dict.fromkeys(dataset_keys))
...
def collect_source_snapshots(current_data: Dict[str, Any] | None) -> List[Dict[str, Any]]:
            ...
            snapshots.append(
                {
                    ...
                    "dataset_label": snapshot.get("label", get_dataset_label(str(dataset_key))),
...
def _inherited_flag_name(field_name: str) -> str:
...
def has_explicit_filter_change(user_input: str, extracted_params: Dict[str, Any], current_data: Dict[str, Any] | None) -> bool:
    ...
    for field_name in APPLIED_PARAM_FIELDS:
        ...
        if normalize_filter_value(new_value) == normalize_filter_value(current_value):
            continue
        if extracted_params.get(_inherited_flag_name(field_name)):
            continue
        if current_value not in (None, "", []) and new_value in (None, "", []) and user_explicitly_mentions_filter(field_name, user_input):
            if any(token in normalized_user_input for token in FILTER_REMOVAL_KEYWORDS):
                return True
        if new_value and user_explicitly_mentions_filter(field_name, user_input):
            return True
        if detect_registered_values(field_name, user_input):
            return True
    return False


def has_required_param_change(
    ...
    current_filters = current_data.get("retrieval_applied_params") or current_data.get("applied_params", {}) or {}
    effective_dataset_keys = dataset_keys or collect_current_source_dataset_keys(current_data)
...
def is_summary_result(current_data: Dict[str, Any] | None) -> bool:
...
def build_current_data_profile(current_data: Dict[str, Any] | None) -> Dict[str, Any]:
    ...
    return {
        ...
        "applied_params": dict(
            (current_data or {}).get("retrieval_applied_params")
            or (current_data or {}).get("applied_params", {})
            or {}
        ),
