...
def _build_analysis_rule_catalog_text() -> str:
    """LLM review 에 넘길 분석 rule 요약 텍스트를 만든다."""

    rules = get_registered_analysis_rules()
    if not rules:
        return "(none)"
    return "\n".join(format_analysis_rule_for_prompt(rule) for rule in rules)


def _review_missing_base_dataset_keys(
    user_input: str,
    chat_history: List[Dict[str, str]],
    current_columns: List[str],
    planned_dataset_keys: List[str],
) -> Dict[str, Any]:
...
{_build_analysis_rule_catalog_text()}
...
{json.dumps(planned_dataset_keys, ensure_ascii=False)}
...
{{
  ...
  "reason": "short explanation"
    ...
    missing_dataset_keys = _dedupe_dataset_keys(
        [key for key in parsed.get("missing_dataset_keys", []) if key not in planned_dataset_keys]
    )
    return {
...
def _finalize_merge_hints(raw_hints: Any, final_dataset_keys: List[str]) -> Dict[str, Any]:
    """최종 dataset 조합에 맞게 merge hint를 정리해 불일치를 막는다."""

    normalized_hints = _normalize_merge_hints(raw_hints)
    if not normalized_hints:
        return {}

    allowed_dataset_keys = {normalize_text(dataset_key) for dataset_key in final_dataset_keys}
    dataset_metrics = {
        dataset_key: columns
        for dataset_key, columns in normalized_hints.get("dataset_metrics", {}).items()
        if dataset_key in allowed_dataset_keys
    }
...
def _is_explicit_dataset_listing_query(
    user_input: str,
    explicit_dataset_keys: List[str],
...
{{
  ...
  "merge_hints": {{
    ...
    keyword_dataset_keys = _dedupe_dataset_keys(
        [
    ...
    coverage_review = _review_missing_base_dataset_keys(
        user_input=user_input,
        chat_history=chat_history,
        current_columns=current_columns,
        planned_dataset_keys=dataset_keys,
    )
    dataset_keys = _dedupe_dataset_keys(
        [
            *dataset_keys,
            *coverage_review.get("missing_dataset_keys", []),
            *[
                dataset_key
                for rule in matched_rules
                for dataset_key in rule.get("required_datasets", [])
    ...
    # LLM이 비워 두었거나 너무 좁게 잡았을 때만 규칙 기반 key를 가산한다.
    if not dataset_keys:
        dataset_keys = list(keyword_dataset_keys)
    elif (
        keyword_dataset_keys
        and len(keyword_dataset_keys) == 1
        and len(dataset_keys) <= 1
        and not parsed.get("needs_post_processing", False)
        and not coverage_review.get("needs_post_processing", False)
    ):
        dataset_keys = list(keyword_dataset_keys)
    elif len(keyword_dataset_keys) >= 2 and len(dataset_keys) <= 1:
        dataset_keys = _dedupe_dataset_keys([*dataset_keys, *keyword_dataset_keys])
    ...
    reviewed_needs_post_processing = bool(
        parsed.get("needs_post_processing", False)
        or coverage_review.get("needs_post_processing", False)
        or matched_rules
    )
    explicit_dataset_listing_request = _is_explicit_dataset_listing_query(
        user_input,
        explicit_dataset_keys,
        matched_rules,
        needs_post_processing=reviewed_needs_post_processing,
    )
    merge_hints = _finalize_merge_hints(parsed.get("merge_hints"), dataset_keys)

    return {
        "dataset_keys": dataset_keys,
        "needs_post_processing": False
        if simple_single_dataset_request or explicit_dataset_listing_request
        else reviewed_needs_post_processing,
