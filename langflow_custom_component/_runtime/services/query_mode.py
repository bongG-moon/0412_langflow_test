...
def choose_query_mode(
    ...
    if has_required_param_change(extracted_params, current_data, requested_dataset_keys):
        return "retrieval"

    if has_explicit_filter_change(user_input, extracted_params, current_data):
        return "retrieval"

    if is_summary_result(current_data) and looks_like_new_data_request(user_input):
        return "retrieval"
