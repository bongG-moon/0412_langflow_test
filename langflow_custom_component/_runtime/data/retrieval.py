...
DATASET_REQUIRED_PARAM_FIELDS = {
    # Most manufacturing snapshots are daily tables today, so `date` is the
    # minimum retrieval boundary for now. If a dataset later needs something
    # else such as factory, site, or shift, add it here and the rest of the
    # runtime will pick it up through `dataset_required_param_fields`.
    "production": ["date"],
    "target": ["date"],
    "defect": ["date"],
    "equipment": ["date"],
...
DATASET_REGISTRY = {
    dataset_key: {
        ...
        "required_param_fields": list(DATASET_REQUIRED_PARAM_FIELDS.get(dataset_key, [])),
    }
...
def dataset_required_param_fields(dataset_key: str) -> List[str]:
...
def dataset_requires_param(dataset_key: str, field_name: str) -> bool:
    """특정 필드가 이 데이터셋의 필수 retrieval boundary 인지 확인한다."""

    normalized_field_name = str(field_name or "").strip()
    if not normalized_field_name:
        return False
    return normalized_field_name in dataset_required_param_fields(dataset_key)
...
def dataset_requires_date(dataset_key: str) -> bool:
    ...
    return dataset_requires_param(dataset_key, "date")
