...
LLM 설정은 별도 공통 config 노드로 만들지 않는다. 각 `LLM JSON Caller`와 `LLM Text Caller` 노드가 자기 input으로 `llm_api_key`, `llm_base_url`, `model_name`, `temperature`, `timeout_seconds`를 직접 받는다. 따라서 같은 flow 안에서도 intent 추출, retrieval planning, pandas code generation, answer generation에 서로 다른 모델명을 입력할 수 있다.
...
- `version`: 사람이 관리하거나 MongoDB 저장 시 생성할 version
- `status`: `draft`, `active`, `archived` 중 하나를 권장
- `metadata`: 설명, timezone, 작성자, 출처 등 부가 정보
- `domain`: 실제 질의 답변 flow가 사용하는 도메인 본문

Phase 1에서는 `domain_id`, `version`, `status`, `metadata`가 없어도 동작하게 한다. 단, `Domain JSON Loader`가 기본값을 채워 표준 wrapper로 정규화해야 한다.
...
- `metadata.timezone`이 없으면 `Asia/Seoul`을 넣는다.
...
- node input으로 API key, base URL, model name을 받는다.
- OpenAI 호환 Chat Completions 형태를 기본으로 한다.
...
각 LLM Caller 노드는 공통 config 연결 없이 자기 input으로 `llm_api_key`, `llm_base_url`, `model_name`, `temperature`, `timeout_seconds`를 받는다.
...
- 각 LLM Caller 노드에 API key, base URL, model name을 직접 input으로 넣을 수 있다.
...
{
  "domain_id": "manufacturing_default",
  "version": "2026-04-17T09:00:00+09:00",
  "status": "draft",
  "source": {
...
MongoDB 저장 시에는 `domain_id`, `version`, `status`, `domain`, `validation`, `source`를 그대로 저장한다.
...
- 기존 active domain을 바로 덮어쓰지 않고 version을 새로 만든다.
...
5. MongoDB upsert와 active version 조회를 붙인다.
