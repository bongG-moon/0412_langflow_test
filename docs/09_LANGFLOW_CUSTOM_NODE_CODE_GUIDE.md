...
LLM API Caller
  -> Data(data={"llm_text": "{\"request_type\":\"data_question\", ...}"})
...
상세한 연결 순서, 각 노드의 입출력 payload, 여러 줄 입력이 MongoDB item 여러 건으로 저장되는 방식, custom LLM 연결 방법은 아래 문서에 따로 정리했다.
...
- LLM 호출은 `Domain Item LLM API Caller`에서 수행하고, prompt는 `prompt_payload` Data output으로 연결한다.
...
- `Query Mode Decider` 이후 12~20번 노드 연결 순서
- `Dummy Data Retriever`와 `OracleDB Data Retriever` 중 하나를 선택해서 연결하는 방법
- custom `LLM API Caller` 노드를 어디에 넣어야 하는지
- `Build Intent Prompt.prompt_payload -> LLM API Caller -> Parse Intent JSON.llm_result` 연결
- `Build Pandas Analysis Prompt.prompt_payload -> LLM API Caller -> Parse Pandas Analysis JSON.llm_output` 연결
- `Build Answer Prompt.prompt_payload -> LLM API Caller -> Final Answer Builder.answer_llm_output` 연결
...
예를 들어 `18_execute_pandas_analysis.md`는 LLM이 만든 pandas 코드를 어떤 기준으로 검증하고 실행하는지 설명하고, `20_final_answer_builder.md`는 최종 응답과 다음 턴 state를 어떻게 만드는지 설명한다.
...
- LLM 호출 구간 앞에는 `prompt_payload` Data output을 만들고, custom LLM caller로 연결한다.
...
## 27. 현재 Main Data Answer Flow 변경 기준

현재 `langflow/data_answer_flow`는 Langflow built-in LLM 노드가 아니라 custom `07 LLM API Caller`를 기본 LLM 호출 노드로 사용한다.

### 27.1 LLM 연결 패턴

같은 `LLM API Caller` component를 canvas에 세 번 배치한다.
...
각 LLM caller는 자체 `llm_api_key`, `model_name`, `temperature`, `response_mode`를 가진다.

`response_mode`는 보통 `auto`로 둔다. prompt builder가 `prompt_type`을 같이 보내기 때문이다.
...
`intent`, `pandas_analysis`는 JSON 응답으로 처리되고, `answer_text`는 자연어 답변으로 처리된다.

### 27.2 Domain 로딩 패턴

Main Flow의 domain source는 `MongoDB Domain Item Payload Loader`다.
...
이 로더는 domain id를 받지 않고 `manufacturing_domain_items` 컬렉션의 active item을 모두 읽는다.

반환 payload의 핵심은 다음 세 가지다.
...
`domain`은 정규화와 pandas 분석에 필요한 전체 domain 정보다.

`domain_index`는 product, process, term, metric, dataset alias를 빠르게 찾기 위한 색인이다.

`domain_prompt_context`는 LLM prompt에 넣기 위한 경량 요약이다. 전체 domain을 prompt에 반복해서 넣지 않아 token 사용량을 줄인다.

### 27.3 Retrieval 노드 패턴

`Retrieval Plan Builder`는 SQL을 만들지 않는다. dataset별 tool 실행 계획만 만든다.
...
`params`는 원본 데이터 조회 전에 필요한 필수 조건이다. `post_filters`는 raw data 조회 후 pandas에서 적용할 조건이다.

`Dummy Data Retriever`와 `OracleDB Data Retriever`는 같은 tool 이름을 사용한다.
...
Oracle 버전은 domain이나 LLM에서 SQL을 받지 않는다. SQL template은 우선 Table Catalog의 dataset별 `sql_template`을 사용하고, table catalog가 없을 때만 `14_oracledb_data_retriever.py` 내부 fallback spec을 사용한다.

### 27.4 노드 설계 원칙

이번 Main Flow 노드들은 아래 기준으로 작성한다.

- 큰 일을 한 노드에 몰아넣지 않는다.
- prompt 생성, LLM 호출, JSON 파싱, 정규화, 조회, pandas 실행, 최종 state 저장을 분리한다.
- 노드 간 데이터는 가능한 `Data(data={...})` 형태로 주고받는다.
- LLM prompt에는 전체 domain을 넣지 않고 경량 context나 관련 slice만 넣는다.
- Oracle 조회는 고정 tool 함수와 TNS connection JSON을 사용한다.
- 최종 Playground 출력은 `answer_message`를 사용하고, 긴 원본 table은 MongoDB에 저장한다.

### 27.5 Table Catalog 분리 패턴

테이블, SQL, DB key, 필수 조회 parameter는 domain knowledge가 아니라 table catalog로 분리한다.

```text
Table Catalog JSON Input
  -> Table Catalog Loader
  -> Main Flow Context Builder
```

권장 입력 형태는 SQL을 그대로 붙여넣는 `sql_template` block 방식이다.

표준 JSON은 raw multiline string을 지원하지 않는다. 그래서 이 프로젝트의 `Table Catalog Loader`는 SQL 전용 확장 문법으로 `"""..."""` 블록을 먼저 안전한 JSON 문자열로 바꾼 뒤 파싱한다.
...
이 방식은 SQL 안에 작은따옴표나 큰따옴표가 있어도 사람이 별도 escaping을 거의 신경 쓰지 않아도 되는 장점이 있다. 단, 엄밀한 JSON 파일은 아니므로 일반 JSON validator가 아니라 `Table Catalog Loader`를 통해 파싱해야 한다.

질문이 어떤 데이터를 필요로 하는지는 두 정보가 함께 결정한다.

- `table_catalog.datasets[].keywords`, `question_examples`: 질문 표현과 dataset 후보를 연결한다.
- domain `metrics[].required_datasets`: 계산 metric이 어떤 dataset들을 필요로 하는지 정의한다.

LLM prompt에는 `table_catalog_prompt_context`만 들어간다. 실제 SQL, table name, db_key는 Oracle 실행 노드가 쓰는 `table_catalog`에만 유지한다.
