...
from manufacturing_agent.app.ui_domain_knowledge import (
...
st.set_page_config(page_title="Manufacturing Chat", layout="wide")
...
def _render_display_options() -> None:
    st.session_state.engineer_mode = st.toggle(
        ...
        help="켜면 pandas 처리 요약과 생성된 코드를 추가로 보여줍니다.",
    )
...
def _render_reset_controls() -> None:
    ...
    with left_col:
        if st.button(
            ...
            st.session_state.chat_reset_notice = (
                "대화를 새로 시작합니다. 이전 질문, 응답, 현재 데이터 조회 조건을 모두 비웠습니다."
            )
            st.rerun()
    ...
    with right_col:
        if st.button(
            ...
            st.session_state.chat_reset_notice = (
                "필터와 현재 결과 테이블을 초기화했습니다. 이전 대화 내용은 그대로 유지됩니다."
            )
            st.rerun()

    st.caption(
        "대화 초기화는 전체 흐름을 새로 시작할 때 사용합니다. "
        "필터 초기화는 이전 질문에서 기억한 날짜/공정/MODE 같은 조건과 현재 결과만 지우고 싶을 때 사용합니다."
    )
...
def _render_chat_page() -> None:
    ...
    st.caption("LangGraph 기반 제조 에이전트를 Streamlit에서 대화형으로 사용할 수 있습니다.")
    ...
    user_input = st.chat_input("예: 어제 D/A3 생산 보여줘")
    if not user_input:
        return
    ...
    st.session_state.messages.append(
        {
            "role": "assistant",
            "content": sanitize_markdown_text(
                rendered_response if isinstance(rendered_response, str) else response
            ),
