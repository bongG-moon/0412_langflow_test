...
def _build_domain_prompt_context(domain: Dict[str, Any], domain_index: Dict[str, Any]) -> Dict[str, Any]:
    ...
    products = {
        key: _short_item(product, ("display_name", "aliases"))
        for key, product in domain.get("products", {}).items()
        if isinstance(product, dict)
    }
    process_groups = {
        key: _short_item(group, ("display_name", "aliases", "processes"))
        for key, group in domain.get("process_groups", {}).items()
        if isinstance(group, dict)
    }
    terms = {
        key: _short_item(term, ("display_name", "aliases", "meaning", "description", "filter", "value"))
        for key, term in domain.get("terms", {}).items()
        if isinstance(term, dict)
    }
...
def load_mongodb_domain_item_payload(db_name_value: Any = None, collection_name_value: Any = None) -> Dict[str, Any]:
    ...
    domain_prompt_context = _build_domain_prompt_context(domain, domain_index)
    document = {"domain_id": "mongodb_domain_items", "status": "active", "metadata": {}}
    return {
        "domain_document": document,
        "domain": domain,
        "domain_index": domain_index,
        "domain_prompt_context": domain_prompt_context,
