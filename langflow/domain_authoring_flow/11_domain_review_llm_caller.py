...
def _read_prompt(value: Any) -> str:
    ...
    for key in ("prompt", "review_prompt", "text"):
...
def call_review_llm(prompt: Any, llm_api_key: Any, model_name: str, temperature: Any, timeout_seconds: Any) -> Dict[str, Any]:
    ...
    try:
        llm = ChatGoogleGenerativeAI(model=model, google_api_key=api_key, temperature=temp, timeout=timeout)
        response = llm.invoke(
            [
...
class DomainReviewLLMCaller(Component):
    display_name = "Domain Review LLM Caller"
    description = "Call an LLM API for semantic review of a domain patch."
    icon = "BrainCircuit"
    name = "DomainReviewLLMCaller"

    inputs = [
        DataInput(name="prompt", display_name="Prompt", info="Output from Build Domain Review Prompt.", input_types=["Data", "JSON"]),
        SecretStrInput(name="llm_api_key", display_name="LLM API Key", info="API key used by this review LLM node.", advanced=True),
        MessageTextInput(name="model_name", display_name="Model Name", info="Model to use for this review LLM node.", value="gemini-flash-latest"),
