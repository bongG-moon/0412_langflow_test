...
def _read_prompt(value: Any) -> str:
    ...
    for key in ("prompt", "domain_structuring_prompt", "text"):
...
def call_llm_json(prompt: Any, llm_api_key: Any, model_name: str, temperature: Any, timeout_seconds: Any) -> Dict[str, Any]:
...
class DomainAuthoringLLMJsonCaller(Component):
    display_name = "Domain Authoring LLM JSON Caller"
    description = "Call an LLM API for domain structuring and return JSON-oriented text."
    icon = "BrainCircuit"
    name = "DomainAuthoringLLMJsonCaller"

    inputs = [
        DataInput(name="prompt", display_name="Prompt", info="Prompt payload from Build Domain Structuring Prompt.", input_types=["Data", "JSON"]),
        SecretStrInput(name="llm_api_key", display_name="LLM API Key", info="API key used by this LLM node.", advanced=True),
        MessageTextInput(name="model_name", display_name="Model Name", info="Model to use for this LLM node.", value="gemini-flash-latest"),
