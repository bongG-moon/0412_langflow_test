...
테이블/컬럼/소스 설명, DDL, 자연어 설명 또는 Table Catalog JSON을 입력하면 Main Flow의 `Table Catalog Loader`가 사용하는 메타데이터 JSON으로 변환한다.
현재 구조에서 Table Catalog는 SQL 실행 정의가 아니라 dataset routing/source metadata다. 실제 SQL은 `get_production_data` 같은 retriever tool 함수 내부에 둔다.

처리 방식:

- JSON이면 LLM 없이 바로 검증
- DDL/자연어/참고 SQL이면 LLM으로 Table Catalog JSON 생성
- 참고 SQL이 입력되어도 `sql_template`, `bind_params`는 저장하지 않고 table name, required params, format params, columns 추론에만 사용
...
MongoDB에 저장된 table catalog item을 조회한다. 목록에서 항목을 선택하면 source metadata, required params, format params, columns, keywords, question examples, 전체 document를 상세 확인할 수 있다.
