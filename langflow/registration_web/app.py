...
def _render_sidebar(settings: Dict[str, Any]) -> str:
    ...
    st.sidebar.markdown(
        ...
        테이블: dataset key, source type, DB key, 필수/format 파라미터, 컬럼, 질문 키워드
        </div>
        """,
        unsafe_allow_html=True,
    )
        ...
        rows.append(
            {
                ...
                "format_params": ", ".join(item.get("format_params", []) if isinstance(item.get("format_params"), list) else []),
...
def render_table_registration_page(settings: Dict[str, Any]) -> None:
    st.title("테이블 정보 등록")
    st.caption("테이블/컬럼/소스 설명 또는 JSON을 Main Flow의 Table Catalog 메타데이터 형식으로 변환합니다. SQL은 retriever tool 함수 내부에서 관리합니다.")

    example = _load_example(
        "table_input_example.txt",
        "생산 데이터\n테이블명: PROD_TABLE\n필수 조건: date\nformat params: date\n컬럼: WORK_DT(date), OPER_NAME(string), production(number)",
    )
    ...
    with st.container(border=True):
        raw_text = st.text_area(
            ...
            key="table_raw_text",
            height=360,
            placeholder="예: 생산 데이터셋을 등록한다.\ndataset_key는 production이다.\nsource_type은 oracle, db_key는 MES이다.\n필수 파라미터와 format_params는 date이다.",
        )
        input_mode = st.radio(
            "처리 방식",
    ...
    if preview_clicked:
        if not raw_text.strip():
            st.warning("등록할 테이블 설명, 컬럼 정보 또는 JSON을 입력해주세요.")
            return
        try:
            ...
            rows.append(
                {
                    ...
                    "format_params": ", ".join(dataset.get("format_params", [])),
...
def render_table_lookup_page(settings: Dict[str, Any]) -> None:
    st.title("테이블 정보 조회")
    st.caption("MongoDB에 저장된 table catalog item을 확인하고, 소스 메타데이터/컬럼/키워드 상세 정보를 봅니다.")
    ...
    format_count = sum(1 for item in table_items if isinstance(item.get("format_params"), list) and item.get("format_params"))
    _render_summary_cards(
        [
            ...
            {"label": "Format Params", "value": f"{format_count}건", "help": "format_params가 등록된 항목", "style": "info"},
        ]
    )
    ...
    with st.container(border=True):
        _render_key_value_grid(
            [
                ...
                ("Format Params", selected.get("format_params", [])),
    ...
    source_tab, column_tab, keyword_tab, raw_tab = st.tabs(["Source / Params", "Columns", "Keywords", "전체 Document"])
    with source_tab:
        st.info("SQL은 table catalog에 저장하지 않습니다. 실제 조회 SQL과 날짜 포맷 변환은 retriever tool 함수 내부에서 관리합니다.")
        st.caption("source metadata")
        st.code(
            _json_dumps(
                {
                    "source_type": selected.get("source_type", ""),
                    "db_key": selected.get("db_key", ""),
                    "table_name": selected.get("table_name", ""),
                    "tool_name": selected.get("tool_name", ""),
                    "required_params": selected.get("required_params", []),
                    "format_params": selected.get("format_params", []),
                }
            ),
            language="json",
        )
