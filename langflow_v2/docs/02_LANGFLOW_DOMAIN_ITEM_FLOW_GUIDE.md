...
이 flow에서 LLM은 크게 두 번 쓰인다.

1. 사용자 의도와 조회 계획을 분류한다.
2. 최종 데이터와 분석 결과를 보고 사용자 답변 문장을 만든다.
...
| 문제 상황 | 먼저 볼 노드 |
| --- | --- |
| LLM에 들어가는 정보가 부족함 | `05 Build Intent Prompt` |
| LLM 호출 자체가 실패함 | `06 LLM JSON Caller` |
| LLM JSON은 맞는데 route가 이상함 | `07 Normalize Intent Plan` |
| canvas 분기가 이상함 | `08 Intent Route Router` |
...
- `09 Dummy Data Retriever`: 테스트용 더미 데이터 생성
- `10 Oracle Data Retriever`: Oracle DB 조회
- `11 MongoDB Data Loader`: 저장된 큰 데이터 재조회
- `12 Current Data Retriever`: 후속 질문에서 현재 데이터 재사용

여러 dataset을 쓰는 경우에는 각 retriever 결과를 `14 Retrieval Payload Merger`에서 하나의 payload로 합친다.
...
| 바꾸고 싶은 것 | 수정 위치 |
| --- | --- |
| LLM에게 보여줄 의도 분류 정보 | `05 Build Intent Prompt` |
| LLM provider, model, api key 처리 | `06 LLM JSON Caller` |
| metric required_datasets 반영 | `07 Normalize Intent Plan` |
| 화면에서 보이는 route 가지 | `08 Intent Route Router` |
| dummy 조회 데이터 | `09 Dummy Data Retriever` |
| Oracle DB 연결과 SQL 실행 | `10 Oracle Data Retriever` |
| 후속 질문 current data 로딩 | `12 Current Data Retriever` |
| pandas 계산 prompt | `17 Build Pandas Prompt` |
| pandas 계획 정규화 | `18 Normalize Pandas Plan` |
| pandas 실행 | `19 Pandas Analysis Executor` |
| 최종 답변 prompt | `22 Build Final Answer Prompt` |
| 답변과 최종 데이터 출력 형태 | `24 Final Answer Builder` |
| 다음 턴 memory 저장 메시지 | `25 State Memory Message Builder` |
