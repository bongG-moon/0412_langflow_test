...
def _apply_column_filters(rows: list[Dict[str, Any]], column_filters: Dict[str, Any]) -> list[Dict[str, Any]]:
...
def _run_job(job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = tool(params)
    result["data"] = _apply_column_filters(result.get("data", []), job.get("column_filters", {})) if isinstance(result.get("data"), list) else []
...
class DummyDataRetriever(Component):
    ...
    inputs = [DataInput(name="intent_plan", display_name="Intent Plan", info="Output from Intent Route Router.single_retrieval or multi_retrieval.", input_types=["Data", "JSON"])]
