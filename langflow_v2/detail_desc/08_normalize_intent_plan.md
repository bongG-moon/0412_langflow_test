...
- `_filters_from_question`: 공정, mode, main flow filter alias를 표준 `filters`로 변환합니다.
...
| `_filters_from_question` | `"WB공정 DDR5"` | `{"process_name": ["W/B1", "W/B2"], "mode": ["DDR5"]}` | 질문에서 표준 필터 조건을 추출합니다. |
