...
## 추가 함수 코드 단위 해석: `build_early_analysis_result`의 skipped 처리
...
finish branch가 선택되지 않은 경우에는 실제 응답을 만들지 않습니다. 대신 뒤쪽 `Analysis Result Merger`가 무시할 수 있는 skipped 결과를 반환합니다.

## 추가 함수 코드 단위 해석: `build_early_analysis_result`의 응답 구성

```python
response = str(plan.get("response") or "질문을 처리하려면 추가 조건이 필요합니다.")
```

normalizer가 만든 clarification 문구가 있으면 사용하고, 없으면 기본 안내 문구를 사용합니다.
...
실제 데이터 조회가 없었으므로 `success`는 false이고 `data`는 빈 list입니다. 다만 최종 답변 노드가 읽을 수 있게 `summary`에 사용자 안내 문구를 넣습니다.
...
조회가 없더라도 이번 intent와 state는 보존합니다. 추후 디버깅과 final result 구성에 필요합니다.
