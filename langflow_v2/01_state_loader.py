...
class StateLoader(Component):
    ...
    description = "Standalone state loader for the simplified manufacturing Langflow flow."
    icon = "ArchiveRestore"
    name = "StateLoader"

    inputs = [
        DataInput(name="chat_input", display_name="Chat Input", info="Connect Chat Input here so message/session metadata can be preserved.", input_types=["Message", "Data", "Text", "JSON"]),
        MessageTextInput(name="user_question", display_name="User Question", info="Manual question fallback when Chat Input is not connected.", advanced=True),
        DataInput(name="previous_state", display_name="Previous State", info="Optional output from State Memory Extractor.previous_state or Final Answer Builder.next_state.", input_types=["Data", "JSON"], advanced=True),
    ...
    def build_state(self):
        ...
        question_value = chat_input if chat_input is not None else getattr(self, "user_question", "")
        payload = load_state(question_value, getattr(self, "previous_state", None), getattr(self, "session_id", "default"))
        state = payload["state"]
        self.status = {
