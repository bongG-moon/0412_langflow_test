...
class BuildPandasPrompt(Component):
    display_name = "V2 Build Pandas Prompt"
    description = "Build the pandas-code prompt from retrieved rows and domain hints."
    icon = "FileCode"
    name = "V2BuildPandasPrompt"

    inputs = [
        DataInput(name="retrieval_payload", display_name="Retrieval Payload", info="Output from V2 Dummy or Oracle Retriever.", input_types=["Data", "JSON"]),
        DataInput(name="domain_payload", display_name="Domain Payload", info="Domain/metric hints for pandas code.", input_types=["Data", "JSON"], advanced=True),
    ]
