...
def build_final_answer(analysis_result_value: Any, answer_text_value: Any = "", output_row_limit_value: Any = "200", display_row_limit_value: Any = "0") -> Dict[str, Any]:
    ...
    if result.get("success"):
        current_data = {
            ...
            "source_required_params": result.get("retrieval_applied_params", plan.get("required_params", {})),
            "source_filters": result.get("retrieval_applied_filters", plan.get("filters", {})),
            "source_column_filters": result.get("retrieval_applied_column_filters", plan.get("column_filters", {})),
    ...
    context.update({"last_intent": plan, "last_retrieval_plan": {"route": plan.get("route"), "jobs": plan.get("retrieval_jobs", [])}, "last_extracted_params": extracted_params, "last_required_params": plan.get("required_params", extracted_params), "last_filters": plan.get("filters", {}), "last_column_filters": plan.get("column_filters", {}), "last_filter_plan": plan.get("filter_plan", []), "last_analysis_summary": result.get("summary", "")})
